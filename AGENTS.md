...
13. **non-contig slice 陷阱（rope 挂死根因）**：引擎调 `fused_rope(q[..., -rope_dim:], None, ...)`，q 是 `[nt, nheads, head_dim]` 的切片，**非连续**（stride[1]=head_dim 不是 rope_dim）。kernel 假设连续会读错内存→q 损坏→下游 attention 挂死/卡住（不 crash，表现 prefill 无响应）。**不能 `.contiguous()`**——rope 必须 in-place 写回原 q_full 内存（`.contiguous()` 写 detached copy，engine 看不到）。解法：kernel 接收 q 的 stride（`launch_fused_rope_strided`），`off = tok*stride_tok + head*stride_head`。同理 k（engine 传 `.unsqueeze(1)` 切片）。**教训**：wrapper 要检查 `tensor.is_contiguous()`，非连续时要么传 stride 给 kernel，要么确认 in-place 语义允许 copy。
